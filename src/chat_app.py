...
def call_handoff(llm_client, handoff_prompt: str, formatted_history: str, gpt_deployment: str) -> str:
    """Call the handoff model using AzureOpenAI client. Handles content filter errors."""
    start_time = time.time()
    with tracer.start_as_current_span("custom_function") as span:
        ...
        try:
            ...
            handoff_response = llm_client.chat.completions.create(
            ...
            log_timing("Handoff Call", start_time, f"Model: {gpt_deployment}")
            return result
        except Exception as e:
...
logger.info(f"Authentication Debug: IS_LOCAL_AUTH='{is_local_auth_env}', is_local_auth={is_local_auth}")
...
if is_local_auth:
    ...
    try:
        ...
        logger.info("Using AzureCliCredential for local authentication")
    except (Exception, CredentialUnavailableError) as e:
        logger.warning(f"AzureCliCredential failed: {e}. Falling back to DefaultAzureCredential")
        project_client = AIProjectClient(
            endpoint=project_endpoint,
    ...
    logger.info("Using DefaultAzureCredential for Azure deployment authentication")
    try:
        # Test DefaultAzureCredential before creating project client
        default_credential = DefaultAzureCredential()
        logger.info("Testing DefaultAzureCredential...")
        default_credential.get_token("https://cognitiveservices.azure.com/.default")
        logger.info("DefaultAzureCredential test successful")
        
        project_client = AIProjectClient(
            endpoint=project_endpoint,
            credential=default_credential,
        )
        logger.info("AIProjectClient created successfully with DefaultAzureCredential")
    except Exception as e:
        logger.error(f"DefaultAzureCredential failed: {e}")
        logger.error("All authentication methods have failed. Check Azure configuration.")
        raise RuntimeError(f"Authentication failed: {e}")

# Test project client by creating a test thread
try:
    logger.info("Testing project client by creating a test thread...")
    test_thread = project_client.agents.threads.create()
    logger.info(f"Test thread created successfully: {test_thread.id}")
    # Clean up test thread
    project_client.agents.threads.delete(test_thread.id)
    logger.info("Test thread deleted successfully")
except Exception as e:
    logger.error(f"Failed to create test thread: {e}")
    logger.error("This indicates authentication is configured but API calls are failing")
    raise RuntimeError(f"API test failed: {e}")

# Log final authentication configuration
logger.info(f"Application startup: IS_LOCAL_AUTH='{os.getenv('IS_LOCAL_AUTH', 'NOT_SET')}', "
           f"Authentication method: {'Azure CLI' if is_local_auth else 'Default Azure Credential'}")
logger.info(f"Azure AI Agent Endpoint: {project_endpoint}")
logger.info("Authentication and API connectivity verified successfully")
...
# Use the same AzureOpenAI client for handoff since ChatCompletionsClient is not working
...
async def debug_auth():
    """Debug endpoint to show authentication configuration"""
    is_local_auth_env = os.getenv("IS_LOCAL_AUTH", "NOT_SET")
    is_local_auth = is_local_auth_env.lower() == "true"
    
    # Get all environment variables starting with specific prefixes for debugging
    env_vars = {}
    for key in os.environ:
        if key.startswith(("IS_LOCAL", "AZURE", "WEBSITE", "CONTAINER", "MSI")):
    ...
    return {
        "is_local_auth_env": is_local_auth_env,
        "is_local_auth_boolean": is_local_auth,
        "authentication_method": "AzureCliCredential" if is_local_auth else "DefaultAzureCredential",
        "azure_ai_agent_endpoint": os.getenv("AZURE_AI_AGENT_ENDPOINT", "NOT_SET"),
        "environment_variables": env_vars,
        "deployment_info": {
            "is_azure_app_service": bool(os.getenv("WEBSITE_HOSTNAME")),
            "is_container_app": bool(os.getenv("CONTAINER_APP_NAME")),
            "has_msi_endpoint": bool(os.getenv("MSI_ENDPOINT")),
        }
    }
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        logger.info(f"WebSocket: IS_LOCAL_AUTH={is_local_auth}, creating threads...")
        
        thread = project_client.agents.threads.create()
        chat_history: Deque[Tuple[str, str]] = deque(maxlen=5)
        customer_loyalty_thread = project_client.agents.threads.create()
        
        logger.info("WebSocket: Threads created successfully")
    except Exception as e:
        logger.error(f"WebSocket: Failed to create threads: {e}")
        logger.error(f"WebSocket: IS_LOCAL_AUTH setting: '{os.getenv('IS_LOCAL_AUTH', 'NOT_SET')}'")
        await websocket.send_text(orjson.dumps({
            "error": "Authentication failed. Please check server configuration.",
            "details": "Unable to create agent threads due to authentication issues."
        }).decode())
